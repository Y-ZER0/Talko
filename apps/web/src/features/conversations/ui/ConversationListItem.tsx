...
export function ConversationListItem({
  ...
  return (
    <div className="relative group">
      ...
      <div ref={menuRef} className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          type="button"
          onClick={(e) => { e.stopPropagation(); setMenuOpen(!menuOpen); setConfirmAction(null); }}
          className="w-7 h-7 rounded-full flex items-center justify-center text-text-muted hover:bg-surface hover:text-text transition-colors"
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full mt-1 z-50 min-w-[180px] bg-surface border border-border rounded-xl shadow-xl overflow-hidden">
            {confirmAction === "delete" ? (
              <div className="p-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-1.5 text-xs rounded-lg bg-surface-muted text-text-muted hover:bg-border transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    disabled={isPending}
                    className="flex-1 px-3 py-1.5 text-xs rounded-lg bg-danger text-white hover:bg-danger/90 transition-colors disabled:opacity-70"
                  >
              ...
              <div>
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); handleAction("leave"); }}
                  className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-danger hover:bg-surface-muted transition-colors"
                >
